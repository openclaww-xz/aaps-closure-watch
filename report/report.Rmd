...
```{r fig-geo, fig.height=6, fig.cap="Largest school-age population changes by tract, 2005-09 to 2020-24 windows. Left: absolute change; right: percent change (tracts with 2009 base of at least 100)."}
...
pA <- ggplot(both, aes(x = delta, y = reorder(tract_name, delta), fill = dir)) +
  ...
  geom_text(aes(label = paste0(ifelse(delta > 0, "+", ""), format(delta, big.mark = ","))), hjust = ifelse(both$delta > 0, -0.15, 1.15), size = 3.1) +
...
pB <- chg %>% filter(!is.na(pct)) %>%
  mutate(extreme = pct <= -30 | pct >= 30) %>%
  filter(extreme | abs(delta) >= 120) %>%
  ggplot(aes(x = pct, y = reorder(tract_name, pct))) +
  geom_col(aes(fill = pct > 0), width = 0.7, show.legend = FALSE) +
  scale_fill_manual(values = c(`TRUE` = pal[5], `FALSE` = pal[2])) +
  geom_text(aes(label = paste0(pct, "%")), hjust = ifelse((chg %>% filter(!is.na(pct)) %>% mutate(extreme = pct <= -30 | pct >= 30) %>% filter(extreme | abs(delta) >= 120) %>% pull(pct)) > 0, -0.3, 1.3), size = 3) +
...
knitr::kable(events, caption = "Consolidated from four research tracks; see Sources.")
```
...
1. AAPS official documents - budget presentations, board materials, bond program updates, personnel contracts (a2schools.org), retrieved September 9, 2026. Full inventory: vault `AAPS-Crisis-2026-sources/official-gov.md`.
